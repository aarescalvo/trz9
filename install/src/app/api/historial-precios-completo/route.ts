    ...
    if (exportar === 'csv') {
      ...
      const rows = historial.map(h => {
        const variacion = h.precioAnterior && h.precioAnterior > 0
          ...
          : 'N/A'
        const pv = (h as any).productoVendible
        const cl = (h as any).cliente
        const op = (h as any).operador
