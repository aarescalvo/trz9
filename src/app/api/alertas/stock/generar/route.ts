...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Check insumos con stock bajo o agotado
    ...
    for (const stock of insumos) {
      const prioridad = stock.cantidadMinima != null && stock.cantidad <= 0 ? 'CRITICA' : 'ALTA'
      const existing = await db.alertaStock.findFirst({
        where: { entidadId: stock.id, tipo: 'STOCK_BAJO', estado: 'ACTIVA' }
      })
      if (existing) {
        await db.alertaStock.update({ where: { id: existing.id }, data: { stockActual: stock.cantidad, prioridad } })
        actualizadas++
      } else {
    ...
    const insumosBajo = await db.stockInsumo.findMany({
      where: { cantidad: { gt: 0 }, cantidadMinima: { not: null } },
      include: { insumo: { select: { nombre: true, codigo: true } } }
    })

    for (const stock of insumosBajo) {
      if (stock.cantidad <= (stock.cantidadMinima || 0)) {
        const existing = await db.alertaStock.findFirst({
          where: { entidadId: stock.id, tipo: 'STOCK_BAJO', estado: 'ACTIVA' }
        })
        if (existing) {
          await db.alertaStock.update({ where: { id: existing.id }, data: { stockActual: stock.cantidad } })
          actualizadas++
        } else {
          await db.alertaStock.create({
            data: {
