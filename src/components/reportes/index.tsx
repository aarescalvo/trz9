...
export function ReportesModule({ operador }: { operador: Operador }) {
  ...
  // Filtros avanzados
  const [filtroRangoFecha, setFiltroRangoFecha] = useState<string>('todos')
  const [busquedaTropa, setBusquedaTropa] = useState<string>('')
  const [busquedaFechaFaena, setBusquedaFechaFaena] = useState<string>('')
  ...
  const [resumen, setResumen] = useState<{
    ...
    tipificadoresActivos: number
  } | null>(null)
  const [rendimientoPorTropa, setRendimientoPorTropa] = useState<any[]>([])
  const [detalleFaena, setDetalleFaena] = useState<any[]>([])
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      // Filtros avanzados
      if (busquedaTropa) params.append('tropaCodigo', busquedaTropa)
      if (busquedaFechaFaena) params.append('fechaFaena', busquedaFechaFaena)
      ...
      if (reportesData.success) {
        ...
        setResumen(reportesData.data.resumen || null)
        setRendimientoPorTropa(reportesData.data.rendimientoPorTropa || [])
        setDetalleFaena(reportesData.data.detalleFaena || [])
      }
          ...
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row gap-4 items-end">
              ...
              <div className="w-full md:w-40">
                <Label className="text-xs text-stone-500">Rango</Label>
                <Select value={filtroRangoFecha} onValueChange={(val) => {
                  setFiltroRangoFecha(val)
                  const hoy = new Date()
                  let desde = ''
                  switch(val) {
                    case 'hoy':
                    ...
                    case 'semana':
                      ...
                      desde = semana.toISOString().split('T')[0]
                      break
                    case 'mes':
                      const mes = new Date(hoy)
                      mes.setMonth(mes.getMonth() - 1)
                      desde = mes.toISOString().split('T')[0]
                      break
                    case 'trimestre':
                      const trim = new Date(hoy)
                      trim.setMonth(trim.getMonth() - 3)
                      desde = trim.toISOString().split('T')[0]
                      break
                    case 'anio':
                      const anio = new Date(hoy)
                      anio.setFullYear(anio.getFullYear() - 1)
                      desde = anio.toISOString().split('T')[0]
                      break
                    default:
                      desde = ''
                  }
            ...
            <Card className="border-0 shadow-md">
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-stone-50 rounded-lg">
                  <div>
                    <Label className="text-xs text-stone-500">Buscar por N° Tropa</Label>
                    <Input
                      placeholder="Ej: B2026"
                      value={busquedaTropa}
                  ...
                  <div>
                    <Label className="text-xs text-stone-500">Buscar por Fecha Faena</Label>
                    <Input
                      type="date"
                      value={busquedaFechaFaena}
                      onChange={(e) => setBusquedaFechaFaena(e.target.value)}
                      className="h-10"
                    />
                  </div>
                  <div className="flex items-end">
                    <Button onClick={fetchAllData} variant="outline" className="h-10">
                      <Filter className="w-4 h-4 mr-2" />
                      Aplicar
                    </Button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    <Card className="bg-amber-50 border-amber-200">
                      <CardContent className="p-3 text-center">
                        ...
                        <p className="text-xl font-bold text-amber-800">{resumen.totalAnimalesFaenados}</p>
                      </CardContent>
                    </Card>
                    <Card className="bg-blue-50 border-blue-200">
                      <CardContent className="p-3 text-center">
                        <p className="text-xs text-blue-600">Peso Vivo</p>
                        <p className="text-xl font-bold text-blue-800">{resumen.totalPesoVivo.toLocaleString()} kg</p>
                      </CardContent>
                    </Card>
                    <Card className="bg-green-50 border-green-200">
                      <CardContent className="p-3 text-center">
                        <p className="text-xs text-green-600">Rinde Promedio</p>
                        <p className="text-xl font-bold text-green-800">{resumen.rindePromedio}%</p>
                      </CardContent>
                    </Card>
                    <Card className="bg-purple-50 border-purple-200">
                      <CardContent className="p-3 text-center">
                        <p className="text-xs text-purple-600">Tropas</p>
                        <p className="text-xl font-bold text-purple-800">{resumen.tropasProcesadas}</p>
                      </CardContent>
                    </Card>
                  </div>
                )}
